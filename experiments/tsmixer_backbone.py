...
class PretrainAnomalyHead(nn.Module):
    ...
    Maintains physical feature independence by processing channels separately.
    ...
    def forward(self, hidden_state: torch.Tensor) -> torch.Tensor:
        ...
        # Apply MLP independently per channel: (B, C, P, 1) -> (B, C, P)
        x = self.mlp(x).squeeze(-1)
        # Pool across physical channels to get final patch logit: (B, P)
        return x.mean(dim=1)
